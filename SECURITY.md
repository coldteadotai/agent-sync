...
5. The published package has zero runtime dependencies and is built from this repository in CI. As of v0.2.0, releases publish through npm Trusted Publishing (OIDC) from a pinned GitHub Actions workflow with provenance attestation; no long-lived npm token capable of publishing this package exists anywhere. (Before v0.2.0, releases were published manually under the maintainer's two-factor authentication.)
