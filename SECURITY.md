...
5. The published package has zero runtime dependencies and is built from this repository in CI. As of the first tag published after Trusted Publishing was configured, releases publish through npm Trusted Publishing (OIDC) from a pinned GitHub Actions workflow with provenance attestation; no long-lived npm token capable of publishing this package exists anywhere. (Releases through 0.2.4 were published manually under the maintainer's two-factor authentication.)
